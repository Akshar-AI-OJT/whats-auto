...
export const api = {
  ...
  whatsapp: {
    ...
    listTemplates: (params: ListWhatsappTemplatesParams = {}) => {
      ...
      const query = qs.toString()
      return protectedRequest<
        Paginated<WhatsappMessageTemplate> | { data?: WhatsappMessageTemplate[]; meta?: PaginationMeta }
      >(`/api/v1/whatsapp/templates${query ? `?${query}` : ''}`, { method: 'GET' })
    },
    ...
    syncTemplates: () =>
      protectedRequest<{ data?: { syncedCount?: number } } & { syncedCount?: number }>(

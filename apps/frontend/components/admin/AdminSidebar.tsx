...
export function AdminSidebar({
  ...
  const { toggleCollapsed } = useAdminChrome()
  ...
  return (
    ...
    >
      ...
      <div className={cn('flex-1 overflow-y-auto py-4', collapsed ? 'px-2' : 'px-3')}>
        <AdminSidebarNav onNavigate={onNavigate} collapsed={collapsed} />
      </div>
      ...
      {!collapsed ? (
        <div className="shrink-0 border-t border-dash-border p-3">
          ...
          >
            ...
            <p className="mt-1 text-sm font-semibold text-ink">{t('sidebar.roleName')}</p>
          </div>
        </div>
      ) : null}
    </aside>

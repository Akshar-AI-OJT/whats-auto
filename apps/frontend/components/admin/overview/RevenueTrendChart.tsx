...
import { MOCK_REVENUE_TREND } from '../mock-data'
...
export function RevenueTrendChart({ className }: { className?: string }) {
  ...
  const { bars, yTicks, max } = useMemo(() => {
    const values = MOCK_REVENUE_TREND.map((d) => d.revenue)
    const peak = Math.max(...values) * 1.1
    const innerW = WIDTH - PAD.left - PAD.right
    const innerH = HEIGHT - PAD.top - PAD.bottom
    const gap = 14
    const barW = (innerW - gap * (MOCK_REVENUE_TREND.length - 1)) / MOCK_REVENUE_TREND.length

    const mapped = MOCK_REVENUE_TREND.map((d, i) => {

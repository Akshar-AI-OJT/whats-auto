import { Clock3, Gift, Target, type LucideIcon } from 'lucide-react'
import { getTranslations } from 'next-intl/server'
import { cn } from '@/lib/utils'

const CARD_KEYS = ['duration', 'personalized', 'free'] as const
...
export async function BookDemoWhy() {
  const t = await getTranslations('bookDemoPage.why')
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute top-1/4 left-0 size-[16rem] rounded-full bg-primary/10 blur-[100px]"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute right-[15%] bottom-0 size-[14rem] rounded-full bg-primary-pale/50 blur-[90px]"
      />
      ...
      <div className="relative z-10 mx-auto max-w-[1200px] px-4 sm:px-6">
        ...
        <ul className="grid grid-cols-1 gap-4 sm:gap-5 md:grid-cols-3 lg:gap-6">
          {CARD_KEYS.map((key) => {

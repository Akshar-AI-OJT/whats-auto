...
export function CampaignCancelDialog({
  ...
  const t = useTranslations('dashboard.campaigns.cancelDialog')
...
export function CampaignPreviewDialog({
  ...
  const t = useTranslations('dashboard.campaigns.previewDialog')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="gap-0 overflow-hidden p-0 sm:max-w-lg" showCloseButton>
        <DialogHeader className="border-b border-dash-border px-5 py-4 text-left sm:px-6">
          <DialogTitle>{t('title')}</DialogTitle>
          <DialogDescription>
            {preview?.campaignName ? t('body', { name: preview.campaignName }) : t('bodyFallback')}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 px-5 py-4 sm:px-6">
          {pending ? (
